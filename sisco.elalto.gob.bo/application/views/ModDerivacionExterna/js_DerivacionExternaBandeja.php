<script>
	...
	/*=============================================
      Section DERIVAR HOJA DE RUTA EXTERNA BANDEJA
	=============================================*/
	$('#div_select_nivel_1').hide();
	$('#div_select_nivel_2').hide();
	$('#div_select_nivel_3').hide();
	$('#load_register_external').hide();
	function derivarHojaRutaExterna(his_id, cor_id, cod) {
		limpiarControlesDerivacionExterna();
		$(".cod_GAMEA").text(cod);
		$("#cor_idExtbandeja").val(cor_id);
		$('#his_idExtbandeja').val(his_id);
		$('#codigoExtbandeja').val(cod);
		...
		if (nivel2.length === 0 && nivel3.length == 0) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					var content = '<option value="0" selected="selected" disabled>Seleccionar Dependencia</option>';
			...
			if (nivel2.length != 0 && nivel3.length == 0) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						var content_n1 = '<option value="0" selected="selected" disabled>Seleccionar Dependencia</option>';
						...
						var content_n2 = '<option value="0" selected="selected" disabled>Seleccionar Dependencia</option>';
		...
		if (niv1_id == nivel1) {
			$.ajax({
				...
				success: function(data) {
					var obj = JSON.parse(data);
					if (obj == '')
						var content = '<option value="0" selected="selected" disabled>No Existe Funcionario Habilitado</option>';
					else
						var content = '<option value="0" selected="selected" disabled>Seleccionar Funcionario</option>';
					for (var i = 0; i < obj.length; i++) {
	...
	$("#form_derivarHojaRutaExterna").submit(function(e) {
		...
		$.ajax({
			type: 'POST',
			url: "<?= site_url('SecretaryControl/derivarHojaRutaExternaBandeja') ?>",
			data: $(this).serialize(),
			beforeSend: function() {
	...
	$('select#select_proveidoDerivacionExterno').on('change', function() {
		$('#a_proveido').val($(this).find(":selected").text() + ' ');
		$('#a_proveido').focus();
		$('#select_proveidoDerivacionExterno').val($('#select_proveidoDerivacionExterno > option:first').val());
	});
