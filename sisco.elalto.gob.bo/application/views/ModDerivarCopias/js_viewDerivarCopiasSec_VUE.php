<script>
	...
	$(function() {
		...
		$("#botonCancelarDerivacion").click(function() {
			window.location = "<?= site_url('SecretaryGestionControl/'); ?>";
		});
	...
	$("#formCrearCopias").submit(function(e) {
		...
		$.ajax({
			type: 'POST',
			url: "<?= site_url('VentanillaUnicaControl/DerivacionPendientesCopias'); ?>",
			data: dataCopias,
			beforeSend: function() {
			...
			success: function(data) {
				if (data == "Derivacion Con Copias Correcta!") {
					...
					Swal.fire({
						type: 'error',
						title: 'Error con la Derivacion con Copias los Datos',
						showConfirmButton: false,
						timer: 2500
					});
				}
			...
			complete: function() {
				setTimeout(function() {window.location = "<?= site_url('SecretaryGestionControl/'); ?>";}, 1000);
				$('#submitCrearCopias').prop('disabled', false);
			},
			dataType: 'html'
		});
	...
	function limpiarSelectDependencia() {
