<script>
	...
	$('#TabHojaRetractar').one('click', TableRetractar);
	/*=============================================
	...
	function TableInicio() {
		...
		HojaRutaActiva = $('#example1').DataTable({
			"processing": true,
			"serverSide": false,
			"ajax": "<?= site_url('VentanillaUnicaControl/fetch_Corresp'); ?>",
			"ordering": true,
			"order": [
				[0, "asc"]
			],
	...
	function TableProceso() {
		...
		HojaRutaActivaProccess = $('#example2').DataTable({
			"processing": true,
			"serverSide": false,
			"ajax": "<?= site_url('VentanillaUnicaControl/fetch_CorrespProcess'); ?>",
			"ordering": true,
			"order": [
				[0, "asc"]
			],
	...
	function TableConcluido() {
		...
		HojaRutaActivaConcluded = $('#example3').DataTable({
			"processing": true,
			"serverSide": false,
			"ajax": "<?= site_url('VentanillaUnicaControl/fetch_CorrespConcluded'); ?>",
			"ordering": true,
			"order": [
				[0, "asc"]
			],
	...
	function TableRetractar() {
		...
		HojaRutaActivaRetractar = $('#example4').DataTable({
			"processing": true,
			"serverSide": false,
			"ajax": "<?= site_url('VentanillaUnicaControl/fetch_CorrespRetractar'); ?>",
			"ordering": true,
			"order": [
				[0, "asc"]
			],
	...
	function Actualizar4Tablas() {
		TableInicio();
		TableProceso();
		TableConcluido();
		TableRetractar();
	}
	function Actualizar1_4Tablas() {
		TableInicio();
		TableRetractar();
	}
	//////////////////////////////////////
