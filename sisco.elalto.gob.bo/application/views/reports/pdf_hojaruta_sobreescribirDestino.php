...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $contenedorSeccion1 .= '</label>
                ...
                <tr>
                    <td colspan=2 width="82%"><b style="color:#fff;">PROCEDENCIA: </b>&nbsp;&nbsp;&nbsp;&nbsp;<span></span></td>                    
                    <td width="18%">
                    <label for="html">';
...
$anchoo=130;
$altoo=4;
$textLengntProcedencia=strlen($procedencia);
if($textLengntProcedencia>64){
    $tamanoFontt=8;
}else{
    $tamanoFontt=9;
}
$pdf->SetFont('helvetica', '',  $tamanoFontt);
$pdf->StartTransform();
$pdf->SetFillColor(255, 255, 255);
$pdf->setCellPaddings(0.1, 0.1, 0.1, 0.1);
$pdf->MultiCell($anchoo,$altoo , $procedencia, 0, 'L', 1, 2, $posicion_xx, $posicion_yy, true);
$pdf->StopTransform();
//////////////////////////////////
if (!empty($derivacioncorrespondencia)) {
    ...
    $positionOrigen = 132;
    $positionY = 132;
    $positiontextfechaaceptadaY = 182;
    $postextofi_Y = 101;
    $postextnom_Y = 108;
    $postextdesc_Y = 101.5;
    $postextobs_Y = 111;
    ...
    foreach ($derivacioncorrespondencia as $key => $valor) {
        ...
        $CantFojas = (null ==$valor->cantidadfojas || 0==$valor->cantidadfojas  ) ? 0 : $valor->cantidadfojas ;
        $CantCD = (null ==$valor->cantidadcd || 0==$valor->cantidadcd ) ? 0 : $valor->cantidadcd ;
        $CantSobres = (null ==$valor->cantidadsobres || 0 ==$valor->cantidadsobres ) ? 0 : $valor->cantidadsobres ;
        $CantCarpetas = (null ==$valor->cantidadcarpetas || 0==$valor->cantidadcarpetas ) ? 0 : $valor->cantidadcarpetas ;
        $CantAnillados = (null ==$valor->cantidadanillados || 0 ==$valor->cantidadanillados) ? 0 : $valor->cantidadanillados ;
       ...
       // $pdf->SetXY(37.6, $positiontextfechaaceptadaY);
        ...
        $pdf->MultiCell(34, 1, 'Cant. Fojas:'.$CantFojas.',Cds:'.$CantCD .',Sobres:'.$CantSobres.', Carpetas:'.$CantCarpetas.', Anillados:'. $CantAnillados, 0, 'L', 1, 2, 13, $positiontextfechaaceptadaY, true);
        $pdf->StopTransform();
        ...
        $pdf->SetFillColor(255, 255, 255);
        $pdf->setCellPaddings(1, 1, 1, 1);
        $pdf->MultiCell(34, 5, $valor->r_oficina_nombre, 0, 'L', 1, 2, 57, $postextnom_Y, true);
