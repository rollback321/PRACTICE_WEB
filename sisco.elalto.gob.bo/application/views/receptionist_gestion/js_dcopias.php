...
<script>
	$(function() {
		...
		$("#botonCancelarDerivacion").click(function() {
			window.location = "<?= site_url('SecretaryGestionControl/crearHojaRuta'); ?>";
		});
	...
	$("#formCrearCopias").submit(function(e) {
		...
		$.ajax({
			...
			complete: function() {
				...
				//setTimeout(window.location = "<?= site_url('SecretaryGestionControl/crearHojaRuta'); ?>",3000);
				$('#submitCrearCopias').prop('disabled', false);
				// $('#loader_process').hide();
			},
			dataType: 'html'
